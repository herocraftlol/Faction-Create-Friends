...
![Version](https://img.shields.io/badge/version-5.15.5-brightgreen) ![Paper](https://img.shields.io/badge/Paper-1.21.x-blue) ![Java](https://img.shields.io/badge/Java-21-orange) ![License](https://img.shields.io/badge/license-MIT-green) ![SQL](https://img.shields.io/badge/liaison_web-sans_SQL-success) ![Build](https://img.shields.io/badge/build-Paper_1.21.4-success)
...
- 🏘️ **Villages autonomes** avec villageois vanilla **recrutés** : Constructeur, Guerrier, Archer, Récolteur, Navigateur, Cheminot
...
- ⚔️ **Serveurs « war zone »** qui ont besoin d'alliances, de guerres, de territoires protégés et de grandes armées de villageois.
...
## 🆕 Nouveautés de la v5.15.5 — *Compilation propre & stabilité* 🛠️⚡📦
...
| Aspect | Avant v5.15.5 | Depuis v5.15.5 |
|---|---|---|
| `mvn clean package` | ⚠️ erreurs de compilation 1.21.4 | ✅ compile du premier coup |
| `EntitySleepEvent` | ❌ supprimé en 1.21.4 | ✅ remplacé par polling `Villager#isSleeping()` |
| `MapPalette.matchColor(Color)` | ❌ signature supprimée | ✅ nouvelle signature `matchColor(r, g, b)` |
| Import `JavaPlugin` | ⚠️ mauvais package dans 2 classes | ✅ `org.bukkit.plugin.java.JavaPlugin` |
| Import `PostType` | ⚠️ manquant dans `Contract` | ✅ ajouté |
| Import `Location` | ⚠️ manquant dans `PlayerTeleportManager` | ✅ ajouté |
| GUI `cmdItem` | surcharge unique | ✅ surcharge 6-args (multi-ligne) ajoutée |
| Lambda sur `view` | ⚠️ variable non finale | ✅ capturée en `final` |
...
- ✅ Plus aucun appel à une classe/événement inexistant, plus d'erreur silencieuse en console.

### 🛠 Corrections du `MapRenderer` (mini-map de faction)

- ✅ `MapPalette.matchColor(Color)` a été supprimé en 1.21.4 → utilisation de la nouvelle signature **`matchColor(r, g, b)`**.
- ✅ Variable `view` capturée en `final` avant le `forEach` qui retire les renderers vanilla — corrige *« local variables referenced from a lambda expression must be final »*.

### 🏷️ Version & message de démarrage

- ✅ Version exposée : **5.15.5**.
...
Tous les fichiers générés dans `plugins/FactionPlugin/` restent **strictement rétro-compatibles**. **Aucune migration à effectuer** : il suffit de remplacer le `.jar`.

### 🔄 Migration depuis v5.15.4

**Aucune action requise.** Il suffit de remplacer le `.jar` et de redémarrer.
...
> ℹ️ **Aucun changement de format de données** : vos fichiers `factions.yml`, `claims.yml`, `bank.yml`, `ranking.yml`, `map.yml`, etc. restent compatibles.
...
Le JAR est produit dans `target/FactionPlugin-5.15.5.jar` (≈ 484 KB).
...
### **v5.15.5** — *Compilation propre & stabilité* 🛠️ *(version actuelle)*
- ✅ `mvn clean package` compile **sans erreur** dès le premier coup sur Paper 1.21.4.
- ✅ Imports `org.bukkit.plugin.java.JavaPlugin`, `PostType`, `Location` corrigés.
- ✅ Soin en dormant via **polling de `Villager#isSleeping()`** (Paper 1.21.4 a supprimé `EntitySleepEvent`).
- ✅ `MapPalette.matchColor(r, g, b)` — nouvelle signature 1.21.4.
- ✅ Lambda capture corrigée (`view` rendu `final` dans `FactionMapManager`).
- ✅ Nouvelle surcharge `cmdItem(Material, name, desc, extra, tip, enabled)` dans `MainMenuGUI`.
- 📦 Aucune migration de données nécessaire — rétro-compatible.

### **v5.15.4** — *Liaison site web sans SQL* 🔌🌐
- 🌐 **`/lier` parle directement au site via HTTP** (`POST /api/faction/push/link-code`) — plus besoin de MySQL.
- 🔑 Authentification simple par header `X-Faction-Key` (clé partagée plugin ↔ `.env` du site).
- ⚙️ Configuration minimale : `site-url` + `faction-api-key`. Section `mysql:` désormais optionnelle.
- ✅ Compilation Paper 1.21.4 propre : `JavaPlugin`, `Material`, `Enchantment`, `PotionEffectType`, `Sound`, `Particle` mis à jour.
- 😴 Soin en dormant : `EntitySleepEvent` (supprimé) → polling `Villager#isSleeping()`.
...
### **v5.15.1** — *Villageois illimités, niveau max 100* ♾️📈
...
- 🖱️ Clic-droit direct sur un villageois recruté, butin de guerre, sommeil réparateur.
